...
void mul(LweSample *result, LweSample *a, LweSample *b, const LweSample *c, const int32_t nb_bits, TFheGateBootstrappingCloudKeySet *keyset) 
{
	...
	LweSample *tmp = new_LweSample_array(16, in_out_params); // A AND B
...
int main(int argc, char **argv) {
	...
	LweSample* anstext1 = new_gate_bootstrapping_ciphertext_array(16, params);
    LweSample* anstext2 = new_gate_bootstrapping_ciphertext_array(16, params);
	...
	MPImul(result, ciphertext1, ciphertext2, ciphertext3, 16, world_rank, bk);
	leftShift(result, result, bk, 16, world_rank);
	...
	if(world_rank % 2 == 0)
	{
		...
		add_full(result1, anstext1, anstext2, ciphertext3, 16, bk);
		...
		cleanup(anstext1, 16, bk);
		cleanup(anstext2, 16, bk);
		cleanup(result1, 16, bk);
	}
	...
	MPI_Reduce(&time_taken, &get_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	...
	delete_gate_bootstrapping_ciphertext_array(16, anstext1);
	delete_gate_bootstrapping_ciphertext_array(16, anstext2);
	delete_gate_bootstrapping_ciphertext_array(16, result1);
